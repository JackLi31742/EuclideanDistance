...
import entities.Tracklet.BoundingBox;
...
public class Neo4jConnector extends GraphDatabaseConnector {
	...
	public Driver driver = GraphDatabase.driver("bolt://172.18.33.37:7687",
...
//	public Session session = driver.session();
    ...
    public Neo4jConnector(Driver driver, Session session) {
		super();
		this.driver = driver;
...
//            list.add(feature.getVector());
	...
	public List<Hour> getHours() {
		Session session = driver.session();
		StatementResult result = session.run("MATCH (n:Hour) RETURN n.hour order by n.hour;");
...
//					System.out.println(camID+","+id+","+startTime+","+feature.getVector());
	...
	//可能不需要
	@Override
	public boolean isaddSimRel(String nodeID1, String nodeID2, double SimRel) {
		// TODO Auto-generated method stub
		Session session = driver.session();
	...
	@Override
	public List<ReIdAttributesTemp> getPersonSimList(String nodeID1) throws NoSuchElementException {
		// TODO Auto-generated method stub
		Session session = driver.session();
		List<ReIdAttributesTemp> list=new ArrayList<>();
		StatementResult result = session.run("MATCH (a:Person {trackletID: {id1}}) -[r:Similarity]-(b:Person) "
				+ "where r.Minute is not null return a.trackletID,b.trackletID,r.Minute;"
	                ,Values.parameters("id1", nodeID1));
		while (result.hasNext()) {
			Record record = result.next();
			ReIdAttributesTemp reIdAttributesTemp=new ReIdAttributesTemp();
			String trackletID1 = record.get("a.trackletID").asString();
			String trackletID2 = record.get("b.trackletID").asString();
			Double sim=record.get("r.Minute").asDouble();
			if (!(trackletID1.equals("null"))) {
				reIdAttributesTemp.setTrackletID1(trackletID1);
			}
			if (!(trackletID2.equals("null"))) {
				...
				reIdAttributesTemp.setSim((Double)sim);
				list.add(reIdAttributesTemp);
//			if (!((sim.equals("null")))) {
//				
//			}
