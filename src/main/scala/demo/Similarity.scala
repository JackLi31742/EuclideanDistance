...
//    val resultListBuf = scala.collection.mutable.ListBuffer.empty[Array[(String, String, Double)]]
    ...
    for(i <- 0 until rddArr.length){
      ...
      everyOneNeedEuDisWithFlann(rddWithIndex,broadcastVar, args)
...
//    return list
  ...
  def everyOneNeedEuDisWithFlann(rddWithIndex: RDD[(ReIdAttributesTemp, Long)],broad:Broadcast[Array[(ReIdAttributesTemp, Long)]]
//          ,dbConnectorByte:Array[Byte]  
//  ,dbConnSingleton:SingletonUtil[GraphDatabaseConnector]
  , args:Array[String]
  )  {
...
if(args(0).equals("minute")){
      rddWithIndex.collect().foreach(f⇒{
    	println("rddWithIndex:-------------------------------")
      println(f)
      })
    val rdd1 = rddWithIndex.mapPartitions(r ⇒ {r.map(r⇒(r._1.getTrackletID,r._1.getFeatureVector))}).repartition(partition)
...
//      val rdd1ArrayRdd=sc.parallelize(rdd1Array)
   ...
   /* var broadArr=broad.value
    ...
    val broadRdd=sc.parallelize(broad.value).map(r ⇒ (r._1.getTrackletID,r._1.getFeatureVector)).repartition(partition)
...
*/     }
      ...
      val rdd1 = rddWithIndex.map(r ⇒ ((r._1.getTrackletID, r._1.getFeatureVector,r._1.getStart)))
   /* rdd1.collect().foreach(f⇒{
    	println("rdd1:-------------------------------")
      println(f)
      })*/
   /* var broadArr=broad.value
    ...
    val broadRdd=sc.parallelize(broad.value).map(r ⇒ ((r._1.getTrackletID, r._1.getFeatureVector,r._1.getStart)))
